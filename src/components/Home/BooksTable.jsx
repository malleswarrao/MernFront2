...
const BooksTable = ({books}) => {
  return (
    ...
    <thead>
        <tr><th className='border border-slate-600 rounded-md'>No</th>
        <th className='border border-slate-600 rounded-md'>Title</th>
        <th className='border border-slate-600 rounded-md'>Author</th>
        <th className='border border-slate-600 rounded-md'>Publish Year</th>
        <th className='border border-slate-600 rounded-md'>Info/Edit/Delete</th>
        </tr> 
        </thead>
        <tbody>
            {books.map((book)=>{book.title})}
            {books.map((book,index)=>{return (<tr key={book._id} className='h-8'>
